...
#Change GFCF_By_Industry_Asset to 3 sector
...
for i in eachindex(rowCode);
    println("i=")
    println(i);
    flowsRowSum[from19ToInd[rowCode19[i]],:] = flowsRowSum[from19ToInd[rowCode19[i]],:]+value.(flows[i,:]);
end=#
insertcols!(flows ,1, :Industry => rowCode19);
splitIndustry = groupby(flows, :Industry);
#sum.(eachcol(flows[1:]))
trial = combine(splitIndustry);
